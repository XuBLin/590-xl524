...
(train_images_h, train_labels_h), (test_images_h, test_labels_h) =\
    cifar100.load_data()
train_images = train_images/np.max(train_images)
train_images_h = train_images_h/np.max(train_images_h)

train_images = train_images.reshape(50000, 32, 32, 3)
train_images_h = train_images_h.reshape(50000, 32, 32, 3)
...
history = model.fit(train_images, train_images, epochs=16, batch_size=1000,
...
for i in range(8):
    ...
    plt.matshow(train_images[int].reshape(32, 32, 3), fignum=False)
    output = model.predict(train_images[int].reshape(1, 32, 32, 3))
    plt.subplot(4, 4, i*2+2)
    plt.matshow(output.reshape(32, 32, 3), fignum=False)
...
for i in range(8):
    int = randint(0, 49999)
    plt.subplot(4, 4, i*2+1)
    plt.matshow(train_images_h[int].reshape(32, 32, 3), fignum=False)
    output = model.predict(train_images_h[int].reshape(1, 32, 32, 3))
    plt.subplot(4, 4, i*2+2)
    plt.matshow(output.reshape(32, 32, 3), fignum=False)

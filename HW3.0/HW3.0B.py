...
loss_val = []
LR = 0.1
max_rand_wb = 1.0     # maximum number for initial guess
GAMMA_L1 = 0.0
...
def model(x, p):
    ...
    if model_type == "linear":
        ...
        for i in range(x.shape[0]):
            product = x[i]
            '''if flag == 0:
                print("first_pro", product)'''
            for j in range(len(layers)-1):
                product2 = np.matmul(product, submatrices[2*j])
                product = product2 + submatrices[2*j+1]
                '''if flag == 0:
                    print("pro2, subm2", product2, submatrices[2*j])
                    print("pro, subm1", product, submatrices[2*j+1])'''
            # product = np.tanh(product)
            pred.append(product[0])
            '''flag = 1'''
        pred = np.array(pred).T
        return pred
...
'''def predict(p):
    ...
    if model_type != "ANN":
        ...
        YPRED_T = np.tan(model(X_norm[train_idx], p))
        YPRED_V = np.tan(model(X_norm[val_idx], p))
        YPRED_TEST = np.tan(model(X_norm[test_idx], p))
...
def minimizer(X, Y, initial_guess, algo='GD'):
...
if model_type != "ANN":
    ...
    count = 0
    for i in range(len(layers)-1):
        count = count + layers[i] * layers[i+1] + layers[i+1]
    po = np.random.uniform(0, 1., size=count)
